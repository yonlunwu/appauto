...
def parse_args():
    parser = argparse.ArgumentParser(description="Script to benchmark concurrent requests to a server.")
    parser.add_argument(
        "--api-key",
        type=str,
        default="AMES_d089c51a130f5635_e900ad283dc6cc030edbd4df854a7581",
        help="api-key to amaas",
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    parser.add_argument(
        "--model-path",
        type=str,
        default="/mnt/data/models/Qwen2.5-7B",
...
async def async_request_sglang_generate(payload, url, pbar: Optional[tqdm] = None, api_key: Optional[str] = None):
    """
...
# def gen_payload(prompt, output_len):
#     payload = {
#         "model": "Qwen2.5-7B-Instruct",  # 改成您实际的模型名称
#         "messages": [
...
def log_to_jsonl_file(data, file_path="performance_metrics.jsonl"):
    ...
    try:
        with open(file_path, "a") as file:
            file.write(json.dumps(timestamped_data) + "\n")  # Write as a single line in JSONL format
    except IOError as e:
...
class WorkloadGenerator:
    def __init__(self, args):
        ...
        init_requests = [
            (i, gen_payload(self.candidate_inputs[i], args.output_length)) for i in range(args.num_clients)
        ]
        self.client_records = {i: {"round": 0, "history": init_requests[i][1]["text"]} for i in range(args.num_clients)}
    ...
    async def handle_request(self, item):
        try:
            ...
            response = await async_request_sglang_generate(payload, self.url, self.pbar, self.api_key)
            ...
            while True:
                if self.sent_requests - self.completed_requests < args.max_parallel:
                    ...
                    if new_request:
                        ...
                        print(f"self.sent_requests: {self.sent_requests}")
                        print(f"self.com_requests: {self.completed_requests}")
                else:
                ...
                if self.distribution == "poisson":
                    ...
                    avg_interval = 1.0 / self.request_rate if self.request_rate > 0 else 1.0
                    sleep_time = random.uniform(0, 2 * avg_interval)
                else:
                    raise ValueError("Invalid distribution type")
                await asyncio.sleep(sleep_time)  # Wait before sending the next request
                # await asyncio.sleep(1)  # Wait before sending the next request
    ...
    def response_handler(self):
        if not self.completed_requests == args.num_clients * args.num_rounds:
            while True:
                try:
                    ...
                    print(f"self.completed_requests: {self.completed_requests}")

                    if self.client_records[client_id]["round"] < args.num_rounds:
                        self.client_records[client_id]["history"] += self.candidate_inputs.pop()
    ...
    def run(self):
        ...
        performance_data = {
            "summary": {
                ...
                "average_ttft": sum(self.performance_metrics["ttft"]) / len(self.performance_metrics["ttft"]),
                "p90_ttft": sorted(self.performance_metrics["ttft"])[int(0.9 * len(self.performance_metrics["ttft"]))],
                "median_ttft": sorted(self.performance_metrics["ttft"])[len(self.performance_metrics["ttft"]) // 2],
                "average_latency": sum(self.performance_metrics["latency"]) / len(self.performance_metrics["latency"]),
...
if __name__ == "__main__":
    ...
    # for request_rate in [0.5]:
    for request_rate in [1]:
        # for request_rate in [5]:
