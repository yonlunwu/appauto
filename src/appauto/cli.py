...
@click.option("--num-gpu-experts", type=str, default=None, show_default=False, help="专家数,仅拉起模型时使用")
def perf(
    ip,
    port,
    ssh_user,
    ...
    if base_ft:
        ...
        try:
            if not skip_launch:
                ft.launch_model_in_thread(model, tp, "perf", port, max_total_tokens=max_total_tokens, num_gpu_experts=num_gpu_experts, wait_for_running=True, timeout_s=launch_timeout)
        ...
        try:
            if not skip_launch:
                amaas.api.launch_model_with_perf(tp, model_store, model, launch_timeout, max_total_tokens, num_gpu_experts)
