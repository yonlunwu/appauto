...
class AMaaSNodeApi(AMaaS):
    ...
    def launch_model_with_perf(
        ...
        if kt_num_gpu_experts:
            ...
            if "--kt_num_gpu_experts" in backend_params:
                backend_params[backend_params.index("--kt_num_gpu_experts") + 1] = str(kt_num_gpu_experts)
            else:
                backend_params.extend(["--kt_num_gpu_experts", str(kt_num_gpu_experts)])
